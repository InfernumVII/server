...
class ConnectionManager:
    def __init__(self):
        ...
        self.recognition_messages_queues: Dict[WebSocket, Deque[str]] = {}
    ...
    async def connect(self, websocket: WebSocket):
        ...
        async with self.lock:
            ...
            self.recognition_messages_queues[websocket] = deque()
    ...
    async def disconnect(self, websocket: WebSocket):
        async with self.lock:
            ...
            if websocket in self.recognition_messages_queues[websocket]:
                del self.recognition_messages_queues[websocket]
    ...
    async def receive_special_message(self, websocket: WebSocket, timeout: float = None):
        ...
        try:
            ...
            return None
    async def receive_recognition_message(self, websocket: WebSocket, timeout: float = None):
        if websocket not in self.recognition_messages_queues:
            return None
            
        try:
            if not self.recognition_messages_queues[websocket]:
                if timeout:
                    await asyncio.wait_for(
                        self._wait_for_recognition_message(websocket),
                        timeout=timeout
                    )
                else:
                    await self._wait_for_recognition_message(websocket)
            return self.recognition_messages_queues[websocket].popleft()
        except asyncio.TimeoutError:
            return None
    ...
    async def _wait_for_special_message(self, websocket: WebSocket):
        while not self.special_message_queues[websocket]:
            await asyncio.sleep(0.1)
    async def _wait_for_recognition_message(self, websocket: WebSocket):
        while not self.recognition_messages_queues[websocket]:
            await asyncio.sleep(0.1)
        ...
        try:
            while True:
                ...
                if message.startswith("SPECIAL:"):
                    ...
                    print(f"REC message received: {message}")
                    async with self.lock:
                        self.recognition_messages_queues[websocket].append(message)
                else:
...
@app.get("/")
async def get():
    with open('templates/final.html', 'r', encoding='utf-8') as f:
...
async def send_special_command_same(client_ids: str, command: str):
    client_list = client_ids.split(',')
...
@app.get("/send-rec-message")
async def send_rec_message(client_id: str, command: str):
    client = manager.clients.get(client_id)
    if client:
        await client.pause_event.wait()  # This will block if pause_event is cleared
    ...
    for websocket in targets:
        try:
            await websocket.send_text(f"rec|{command}")
            spc = await manager.receive_recognition_message(websocket, timeout=1000000000000000)
            #print(spc)
            if spc:
                for client in manager.clients.values():
                    if client.websocket == websocket:
                        break
                #print("spc", spc)
                results.append(f"{spc[4:]}")
                raw_rec = spc.split("**")
                if len(raw_rec) > 2:
                    raw_rec = raw_rec[2:]
                    for i in range(len(raw_rec)):
                        manager.marks_dict[i] = raw_rec[i].split(" ")
...
async def get_battery(client_id : str):
    return await send_special_command(client_id, "battery.battery_level")
...
tasks = {
    '0': [
        ...
        {"command": "await navigate_wait(z=1, frame_id='body', auto_arm=True)", "timing": 1},
        {"command": "await navigate_wait(x=0, y=0, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"command": "startRecogniton()", "timing": 1},
        {"command": "set_led_orange()", "timing": 1},

        {"command": "await navigate_wait(x=0, y=5, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"command": "await navigate_wait(x=1.1, y=5, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"command": "set_led_blue()", "timing": 1},
        {"command": "await navigate_wait(x=1.1, y=0, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"command": "await navigate_wait(x=1.1, y=0, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"command": "await navigate_wait(x=0, y=0, z=1.5, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"command": "stopRecognition()", "timing": 1},
        {"command": "await land_wait()", "timing": 1},
        {"command": "getRecognizedMarks()", "timing": 1},
    ],
    '1': [
        {"command": "stopRecognition()", "timing": 1},
        {"command": "await navigate_wait(z=1, frame_id='body', auto_arm=True)", "timing": 1},
        {"command": "await navigate_wait(x=3.6, y=0, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"command": "startRecogniton()", "timing": 1},
        {"command": "set_led_orange()", "timing": 1},
        
        {"command": "await navigate_wait(x=3.6, y=5, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"command": "await navigate_wait(x=2.5, y=5, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"command": "set_led_blue()", "timing": 1},
        {"command": "await navigate_wait(x=2.5, y=0, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"command": "await navigate_wait(x=2.5, y=0, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"command": "await navigate_wait(x=3.6, y=0, z=1.5, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"command": "stopRecognition()", "timing": 1},
        {"command": "await land_wait()", "timing": 1},
        {"command": "getRecognizedMarks()", "timing": 1},
    ],
    ...
    '4': [
        ...
        {"command": "startRecogniton()", "timing": 1},
        {"command": "set_led_blue()", "timing": 1},
        {"command": "await navigate_wait(x=0, y=2, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"command": "set_led_orange()", "timing": 1},
        {"command": "await navigate_wait(x=0, y=0, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"command": "await land_wait()", "timing": 1},
    ]
...
async def get_devices():
    for i in manager.clients.keys():
        ...
        try:
            ...
            manager.clients[i].ready = True
        except:
            percent, voltage = "0", "0"
            manager.clients[i].ready = False
...
async def run_task(client_ids: str): #, tasks: Dict[int: List]
    visited_marks = []
    r = await send_command_same("1", "stopRecognition()", "0.1")
    r = await send_command_same("0,1", "set_led_orange()", "0.1")
    r = await send_command_same("0,1", "await navigate_wait(z=1, frame_id='body', auto_arm=True)", "1")

    commands = [
        {"id": "0", "command": "await navigate_wait(x=0, y=0, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"id": "1", "command": "await navigate_wait(x=3.6, y=0, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1}
    ]

    r = await send_commands("0,1", commands)
    r = await send_command_same("0", "startRecogniton()", "0.1")

    '''commands = [
        {"id": "0", "command": "await navigate_wait(x=0, y=1.5, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"id": "1", "command": "await navigate_wait(x=1.8, y=0, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1}
    ]

    await send_commands("0,1", commands)'''
    Y = [0, 1.5, 2.5, 3.5, 4.5, 5.3]
    for y_ind in range(1, len(Y)):
        r = await send_command_same("0", f"await navigate_wait(x=0, y={float(Y[y_ind])}, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "1")
        r = await send_command_same("1", f"await navigate_wait(x=1.8, y={float(Y[y_ind-1])}, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "1")

        r = await send_command_same("0", f"await navigate_wait(x=3.6, y={float(Y[y_ind])}, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "1")

        if len(manager.marks_dict.keys()) != 0:
            if len(manager.marks_dict.keys()) != len(visited_marks):
                r = await send_command_same("0", f"await navigate_wait(x=3.6, y={float(Y[y_ind])}, z=2.8, yaw=(math.pi/2), frame_id='aruco_map')", "1")

            for k,v in manager.marks_dict.items():
                if k not in visited_marks:
                    visited_marks.append(k)
                    r = await send_command_same("1", f"await navigate_wait(x={float(v[0])}, y={float(v[1])}, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "1")
                    r = await send_command_same("1", f"set_led_blue()", "1")
                    r = await send_command_same("1", f"await navigate_wait(x={float(v[0])}, y={float(v[1])}, z=1.5, yaw=(math.pi/2), frame_id='aruco_map')", "1")
                    r = await send_command_same("1", f"set_led_orange()", "1")

        r = await send_command_same("1", f"await navigate_wait(x=1.8, y={float(Y[y_ind-1])}, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "1")
        r = await send_command_same("0", f"await navigate_wait(x=3.6, y={float(Y[y_ind])}, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "1")

    commands = [
        {"id": "0", "command": "await navigate_wait(x=0, y=5.3, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"id": "1", "command": "await navigate_wait(x=3.6, y=4.5, z=2, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1}
    ]
    r = await send_commands("0,1", commands)
    r = await send_command_same("0", "stopRecognition()", "0.1")
    commands = [
        {"id": "0", "command": "await navigate_wait(x=0, y=0, z=1.5, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1},
        {"id": "1", "command": "await navigate_wait(x=3.6, y=0, z=1.5, yaw=(math.pi/2), frame_id='aruco_map')", "timing": 1}
    ]
    r = await send_commands("0,1", commands)
    commands = [
        {"id": "0", "command": "await land_wait()", "timing": 1},
        {"id": "1", "command": "await land_wait()", "timing": 1}
    ]
    r = await send_commands("0,1", commands)
